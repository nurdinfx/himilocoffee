export const getImageUrl = (imagePath) => {
  ...
  if (imagePath?.includes('localhost') || imagePath?.includes('127.0.0.1')) {
    // If it's a relative path hidden inside a localhost URL, extract it
    const match = imagePath.match(/(\/(public|images)\/.*)/);
    if (match) {
        return `https://himilocoffee.onrender.com${match[1]}`;
    }
